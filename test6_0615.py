...
from mpiigaze import gaze_estimate,head_ROI
import socket
import struct
import pickle
...
client_socket.connect(('192.168.219.102', 8485))
...
# UI data -> leaving time, drowsy, focus roi, frame counter
...
while(True):
    ...
    if(ret is True) and (current_time >1./FPS):
        ...
        if not rects:
            ...
            x,y,z,distance,pitch,yaw = gaze_mpii._run_get_image(frame)
            #print(x,y,z,distance,pitch,yaw)
        ...
        for rect in rects:
            ...
            face_center = [int((rect.right()+ rect.left())/2),int((rect.top()+ rect.bottom())/2)]
            
            print("face center",face_center)
            space_out,focus_iou = ROI.predictor(face_center,x,y,z,distance,pitch,yaw)
            cv2.line(frame, tuple(face_center), tuple(face_center), (0,0,155),2)
        ...
        #cv2.putText(frame, msg_IOU , (10,400), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

        print(round(time.time()-video_start,2),"s progressing" )
        """
        if time.time()-video_start >= 60*cnt:
            ...
            frame_IOU = (UI_total_data[2] / UI_total_data[3])*100 if cnt >=1 else 0
            msg_focus = "focus in computer " + str(int(frame_IOU))+"%"
            ...
            frame_IOU = (UI_data_prev[2] /UI_data_prev[3])*100 if cnt >=1 else 0
            msg_focus = "focus in computer " + str(int(frame_IOU))+"%"
